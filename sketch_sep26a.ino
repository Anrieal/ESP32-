...
// 九个状态
const char* states[] = {"折纸", "赤空", "玉兰", "水光", "风荧", "玄冰", "月海", "日珥", "星灯"};
// 蓝牙键盘初始化（自定义蓝牙名称和制造商）
    ...
    do {
        ...
        if (isTimeSynchronized) {
            ...
            if (getLocalTime(&timeinfo)) {
              ...
              // 计算焉虚历年和日
              int epoch_seconds_per_day = 1200;
              int epoch_days_per_year = 64;
              long total_days = unixTime / epoch_seconds_per_day;
              int year = total_days / epoch_days_per_year;  // 计算焉虚历年
              int day = total_days % epoch_days_per_year;   // 计算焉虚历日
              int secondsToday = unixTime % epoch_seconds_per_day;   // 计算当日已经过的秒数

              // 计算当前状态
              const char* currentState = states[day % 9];  // 根据天数轮换状态
